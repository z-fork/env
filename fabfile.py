...
def base_env():
    ...
    >fdisk -l
    >fdisk /dev/vdb1  # n p 1 \n \n wq
    >fdisk -l
    >mkfs.ext3 /dev/vdb1
    >mkdir /idata
    >mount /dev/vdb1 /idata
    >echo '/dev/vdb1 /home ext3 default 0 0' >> /etc/fstab
    >cat /etc/fstab

    FAQ: Could not retrieve mirror...
...
def yum_install():
    package_list = ['vim', 'gcc', 'gcc-c++', 'make', 'tmux', 'git', 'libevent-devel.x86_64',
                    ...
                    'pcre-devel', 'freetype', 'freetype-devel', 'libpng-devel', 'blas-devel',
                    'lapack-devel']
...
def setup_nginx():
    ...
    pcre_source_url = 'http://www.mirrorservice.org/sites/dl.sourceforge.net/pub/sourceforge/p/pc/pcre/pcre/8.12/{file}'.format(file=pcre_file)
    with cd(FILE_DIR):
...
def copy_supervisord_service():
    Rsync().sync(CONFIG_DIR + '/supervisord/hunter-supervisord', '/home/mongoo/hunter-supervisord')
    sudo('mv /home/mongoo/hunter-supervisord /etc/init.d/hunter-supervisord')
    sudo('chmod a+x /etc/init.d/hunter-supervisord')

    remote_supervisor_dir = '/home/mongoo/local/supervisor/'
...
def mysql_env():
    ...
    # copy_mysql_cnf()
    # change_data_dir()
    # link_libs()  # for x64 centos

    # clean '' user and change root passwd
    # delete from user where User='';
    # UPDATE user SET Password=PASSWORD('qZePiMM3S34j3wW89MUEPcP9S') where User='root';
    # FLUSH PRIVILEGES;
...
def remove_default():
    ps = sudo('rpm -qa | grep -i mysql | xargs')
    for p in ps.splitlines():
        sudo('yum -y remove %s' % p)
...
def link_libs():
    sudo('ln -s /usr/lib64/mysql/libmysqlclient* /usr/lib64/')
